...
get_site_data <- function(city_names) {
  ...
  #-----------------------------------------------------------------------------
  # get local data and isolation data
  ...
  for(i in 1:length(city_names)){
    ...
    select(city, new_id, total_area_sqm, total_green_space_area, isolation, tree_percent_cover, grass_shrub__percent_cover) %>%
    ...
    temp <- temp %>%
      ...
      mutate(n_plant_genera = replace_na(n_plant_genera, 0),
             ...
             plant_genera_density = log_n_plant_genera / log(total_area_sqm)) %>%
      mutate(plant_genera_density_scaled = center_scale(plant_genera_density))
    
    # add city latitude to site data
    # first read the data 
    latitude_data <- read.csv(paste0(
      "./data/city_latitude.csv"
    ))
    
    temp <- left_join(temp, latitude_data, by = "city")
  ...
  for(i in 1:length(city_names)){
    ...
    mutate(proportion_landscape_vegetation_scaled = center_scale(proportion_landscape_vegetation),
           proportion_landscape_open_developed_scaled = center_scale(proportion_landscape_open_developed),
           proportion_landscape_medhigh_developed_scaled = center_scale(proportion_landscape_medhigh_developed),
           proportion_landscape_woody_scaled = center_scale(proportion_landscape_woody),
           proportion_landscape_grassherb_scaled = center_scale(proportion_landscape_grassherb)
    )
