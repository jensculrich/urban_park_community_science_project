...
for(city_number in 1:n_cities){
  ...
  temp <- cbind(as.data.frame(
      readRDS(paste0(
        "./model_outputs/stan_out_", city, "_2km_connectivity_family.rds"))
  ), city)
...
pre_p <- ggplot(df) + 
  stat_smooth(aes(x = pred_data, y = rel_mean, group=city), method = "loess", se = FALSE) +
  stat_smooth(aes(x = pred_data, y = rel_lower_95, group=city), method = "loess", se = FALSE) +
  stat_smooth(aes(x = pred_data, y = rel_upper_95, group=city), method = "loess", se = FALSE) +
  stat_smooth(aes(x = pred_data, y = rel_lower_50, group=city), method = "loess", se = FALSE) +
  stat_smooth(aes(x = pred_data, y = rel_upper_50, group=city), method = "loess", se = FALSE)
pre_p
...
(p  <- ggplot(data = df2, aes(pred_data, group=city)) +
   ...
   ylim(c(0, 1)) +
   theme_classic() +
   xlab("Park Size (st dev's from avg. park size within city)") +
   ylab("Proportion of Regional Species Pool\nExpected to Occur") +
...
(p  <- ggplot(data = df2, aes(original_scale_data)) +
    ...
    geom_ribbon(aes(ymin = lower_95, ymax = upper_95), fill = "lightskyblue2", alpha = 0.3) +
    geom_ribbon(aes(ymin = lower_50, ymax = upper_50), fill = "lightskyblue3", alpha = 0.3) +
    #geom_ribbon(
