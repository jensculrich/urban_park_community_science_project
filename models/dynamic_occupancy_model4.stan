...
// Updated model that includes an effect of family, with species nested within.

// jcu, started june, 2024.
...
data {
  ...
  int<lower=0> n_families; // number of species
  int<lower=1> family_lookup[n_species]; // vector of species identities
...
parameters {
  ...
  vector[n_species] p_species; // species specific intercept for detection rate
  real<lower=0> sigma_p_species; // variance in species intercepts
  vector[n_families] p_family; // family specific intercept for detection rate
  real<lower=0> sigma_p_family; // variance in family intercepts
...
transformed parameters {
  ...
  // Phylogenetic clustering for detection
  // compute the varying intercept at the level-2 species level
  // by clustering within Level-3 (n_families level-3 random intercepts)
  
  vector[n_species] p_species_nested; // phylogenetically nested intercepts
  
  for(i in 1:n_species){
    p_species_nested[i] = p_family[family_lookup[i]] + p_species[i];
  }
      ...
      for(k in 1:n_years){ // loop across all years
        for(l in 1:n_surveys){ // loop across all surveys
          ...
          p[i,j,k,l] = inv_logit( // probability (0-1) of detection is equal to..
            p0 + // a global intercept
            p_species_nested[species[i]] + // a species specific intercept
...
model {
  ...
  sigma_p_species ~ normal(0, 2);
  p_family ~ normal(0, sigma_p_family);
  sigma_p_family ~ normal(0, 2);
