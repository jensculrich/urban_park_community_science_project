...
transformed parameters {
  ...
  p_city = p0 + sigma_p_city * p_city_raw;
        ...
        psi1 = inv_logit( // probability (0-1) of occurrence in year 1 is equal to..
          psi1_city[city_id_vector] +
          psi1_species[species_integer_vector] + // a species specific intercept
          psi1_wingspan[city_id_vector] * wingspan[species_integer_vector] + // a species effect of migratory
          psi1_park_size[city_id_vector] * park_size[multicity_site_id_vector] + // a site effect of park size
          psi1_isolation[city_id_vector] * isolation[multicity_site_id_vector] // a site effect of park isolation
          ); // end psi1[r]
        
        gamma = inv_logit( // probability (0-1) of colonization is equal to..
          gamma_city[city_id_vector] +
          gamma_species[species_integer_vector] + // a species specific intercept
          gamma_wingspan[city_id_vector] * wingspan[species_integer_vector] + // a species effect of migratory
          gamma_park_size[city_id_vector] * park_size[multicity_site_id_vector] + // a site effect of park size
          gamma_isolation[city_id_vector] * isolation[multicity_site_id_vector] // a site effect of park isolation
          ); // end gamma[i,j,k]
                
        phi = inv_logit( // probability (0-1) of persistence is equal to..
          phi_city[city_id_vector] +
          phi_species[species_integer_vector] + // a species specific intercept
          phi_wingspan[city_id_vector] * wingspan[species_integer_vector] + // a species effect of migratory
          phi_park_size[city_id_vector] * park_size[multicity_site_id_vector] + // a site effect of park size
          phi_isolation[city_id_vector] * isolation[multicity_site_id_vector] // a site effect of park isolation
          ); // end phi[i,j,k]
  ...
  array[R] real psi;
        ...
        if(site_survey_year_vector < 2){ // define initial state
          psi = psi1; 
          ...
          psi = psi[prev_index_vector] * phi[prev_index_vector] + 
                    (1 - psi[prev_index_vector]) * gamma[prev_index_vector]; 
        } // end if/else
