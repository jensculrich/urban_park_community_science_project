...
// I've included a species-specific month-phenology covariate
      ...
      for (k in 1:n_years){
          ...
          if (sum(V[i,j,k]) > 0){ // lp observed 
            ...
            target += (log_sum_exp(log(psi[i,j,k]) + log1m(p[i,j,k,1]) + log1m(p[i,j,k,2]) + 
                                                    ...
                                                    log1m(p[i,j,k,7]) + log1m(p[i,j,k,8]) +
                                                    log1m(p[i,j,k,9]) + log1m(p[i,j,k,10]) +
                                                    log1m(p[i,j,k,11]) + log1m(p[i,j,k,12]),
