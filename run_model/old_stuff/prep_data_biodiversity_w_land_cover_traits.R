...
prep_data <- function(min_species_detections,
  ...
  parks_shp <- read_sf("./data/Parkserve_Shapefiles_05212024/ParkServe_Parks.shp")
  ...
  parks2 <- sf::st_intersection(parks_shp, los_angeles_shp)
  
  parks2 <- parks2[sample(nrow(parks2), n_parks_sampled_per_city), ]
  
  # let's add a buffer around each park and then merge parks that are touching or overlapping
  parks2 <- st_buffer(parks2, buffer_distance) %>%
    filter(Park_Size_ != 0)
  ...
  df <- df %>%
    ...
    group_by(species) %>%
    mutate(n_binary_detections_per_species=n_distinct(ParkID, year)) %>% 
    add_tally() %>%
    # filter out species below some minimum number of detections
    # right now, this is detections across ALL cities
    filter(n_binary_detections_per_species >= min_species_detections) %>% 
    rename("species_detections" = "n_binary_detections_per_species") %>%
    ungroup()
