...
prep_data <- function(city_names,
  ...
  if(family_sampling == TRUE){
    temp <- df %>%
      ...
      group_by(family, city, year, new_id, survey) %>%
      add_tally() %>%
      ungroup() %>%
    ...
    temp <- df %>%
      # if the species was sampled than at least that species was sampled (may also be a comm sample)
      mutate(non_comm_sample = 1) %>%
      group_by(city, year, new_id, survey) %>%
      add_tally() %>%
      ungroup() %>%
      mutate(mean_species_per_comm_sample = mean(n))
  ...
  for(k in 1:n_years){
    for(l in 1:n_surveys){
      ...
      temp <- df %>%
        ...
        dplyr::select(species, multicity_site_id) %>% # if you don't deselect the other variables it expands the temp matrix for every unique one
        ...
        full_join(., as.data.frame(site_vector), by=c("multicity_site_id" = "site_vector")) %>% 
        
        # group by SPECIES
        group_by(species) %>%
        mutate(row = row_number()) %>%
        # spread sites by species, and fill with 0 if species never captured this interval*visit
        spread(multicity_site_id, row, fill = 0) %>%
  ...
  if(family_sampling == TRUE){
    ...
    community_samples <- df %>%
      ...
      group_by(family, multicity_site_id, year, survey) %>%
      ...
      group_by(multicity_site_id, family, year, survey) %>%
      slice(1) %>%
      ungroup()
  } else{
    ...
    community_samples <- df %>%
      ...
      # determine whether a community sampling event occurred
      # using sampling information from other species in same survey interval 
      group_by(multicity_site_id, year, survey) %>%
      mutate(n_species_sampled = n_distinct(species)) %>%
      ungroup() %>%
  ...
  all_species_site_visits <- as.data.frame(cbind(
    ...
    rename("species" = "V1",
           "multicity_site_id" = "V2",
           "year" = "V3",
           "survey" = "V4",
           "family" = "V5") %>%
    mutate(year = as.integer(year),
           survey = as.integer(survey),
           multicity_site_id = as.integer(multicity_site_id))
  ...
  if(family_sampling == TRUE){
    ...
    all_visits_joined <- left_join(
      ...
      by=c("multicity_site_id", "family", "year", "survey")) %>%
    ...
    all_visits_joined <- left_join(
      all_species_site_visits, community_samples, 
      by=c("multicity_site_id", "year", "survey")) %>%
