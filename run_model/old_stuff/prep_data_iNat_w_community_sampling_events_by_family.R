...
prep_data <- function(min_species_detections,
  ...
  df <- st_transform(df_sf, crs = crs) %>%
    ...
    # mutate to lower case necessary to keep everything in alphabetical order
    mutate(PARK_NAME = tolower(PARK_NAME))
  ...
  df <- df %>%
    ...
    dplyr::select(species, family, PARK_NAME, survey, year) %>%
