...
#For Michigan, we also need data from Essex County, Ontario
leps_data_essex <-fread("data/inat_data/01_raw_data/iNat_leps_essex.csv")
leps_data_MI <- leps_us_data%>%
  filter(stateProvince=="Michigan")%>%
  rbind(leps_data_essex)
...
write.csv(leps_data_IL, "data/inat_data/02_filtered_data/Leps/leps_data_IL.csv")
