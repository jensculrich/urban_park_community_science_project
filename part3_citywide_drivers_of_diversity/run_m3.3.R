...
mean_richness <- simmed_diversity[[8]] # 8 is beta diversity replacement 
...
fitg_cv <-  varsel(m3.3, method = "L1", nterms_max = 2, nclusters_pred = 10,
                   seed = 5555)
...
#---------------------------------------------------------
m3.3.1 <- rstanarm::stan_glm(mean ~ percent_tree_scaled + 
                               log_IIC_scaled, 
                             data = city_data)
summary(m3.3.1)

plot(m3.3.1)
pp_check(m3.3.1) # ?bayesplot::ppc_hist

# 80% interval of estimated reciprocal_dispersion parameter
posterior_interval(m3.3.1, pars = "log_park_size_scaled", prob = 0.5)
plot(m3.3.1, "areas", pars = "percent_tree_scaled", prob = 0.9)
...
mean_richness <- simmed_diversity[[8]] 
...
for(i in 1:n_models){
  ...
  fit <- rstanarm::stan_glm(response ~ percent_tree_scaled +
                              log_IIC_scaled, 
                            data = city_data)
  ...
  percent_tree_scaled[(n_subsamples*(i-1)+1):((n_subsamples*(i-1))+n_subsamples)] <- draws[sample_rows,2]
...
posterior_draws <- as.data.frame(cbind(intercept, percent_tree_scaled, log_IIC_scaled, sigma))
# plot the sample densities
mcmc_areas <- mcmc_areas(posterior_draws, 
                         pars = c("intercept", "percent_tree_scaled", "log_IIC_scaled")) +
  #labs(title = "Posterior Densities of Retained Predictors") +
  theme_classic() +
  scale_x_continuous(name = "Posterior Model Estimate") +
  scale_y_discrete(labels = c("Intercept", "% Tree Cover", "Mean Isolation")) +
...
base <- ggplot(city_data, aes(x = percent_tree_scaled, y = mean)) +
  ylab("Species Dissimilarity\n(Species Replacement Differences)") +
  xlab("% Tree Cover") +
...
m3.3_plot <- cowplot::plot_grid(mcmc_areas, base, base2, ncol = 3, rel_widths = c(1, 1, 1))

saveRDS(m3.3_plot, "./part3_citywide_drivers_of_diversity/figures/m3_plots/m3.3_plot.rds")
