...
fitg_cv <- varsel(m3.1, method = "L1", nterms_max = 2, nclusters_pred = 10,
                   seed = 5555)
...
#---------------------------------------------------------
m3.1.sel <- rstanarm::stan_glm(mean ~ log_park_size_scaled +
                                 percent_tree_scaled, 
                             data = city_data)

plot(m3.1.sel)
pp_check(m3.1.sel) # ?bayesplot::ppc_hist
...
posterior_draws <- as.data.frame(cbind(intercept, log_park_size_scaled, percent_tree_scaled, sigma))
# plot the sample densities
mcmc_areas <- mcmc_areas(posterior_draws, 
           pars = c("intercept", "log_park_size_scaled", "percent_tree_scaled")) +
  #labs(title = "Posterior Densities of Retained Predictors") +
  theme_classic() +
  scale_x_continuous(name = "Posterior Model Estimate") +
  scale_y_discrete(labels = c("Intercept", "Median log(Park Size)", "% Tree Cover")) +
...
# plot on a predictive scale
base2 <- ggplot(city_data, aes(x = percent_tree_scaled, y = mean)) +
  ylab("Median Species Richness") +
  xlab("% Tree Cover") +
  scale_color_manual(values=my_palette, labels = city_names_labels, name="City") + 
  theme_classic() + 
