...
m3.2.0 <- rstanarm::stan_glm(mean ~ 1, 
                           data = city_data)
...
a5 <- mcmc_areas(projdraws, pars=c("(Intercept)",vsel), prob = 0.9)
...
posterior_interval(m3.2.1, pars=c("(Intercept)",vsel), prob = 0.9)
plot(m3.2.1, "areas", pars=c("(Intercept)",vsel), prob = 0.9)
...
percent_semi_natural_scaled <- vector(length = n_subsamples*n_models)
sigma <- vector(length = n_subsamples*n_models)

for(i in 1:n_models){
  city_data$response <- mean_prop_disturbance_avoidant[,i]
  
  fit <- rstanarm::stan_glm(response ~ percent_semi_natural_scaled, 
                               data = city_data)
  ...
  percent_semi_natural_scaled[(n_subsamples*(i-1)+1):((n_subsamples*(i-1))+n_subsamples)] <- draws[sample_rows,2]
