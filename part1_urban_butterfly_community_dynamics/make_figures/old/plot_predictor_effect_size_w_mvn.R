...
stan_out2 <- readRDS("./model_outputs/stan_out_2km_connectivity_family_mvn.rds")
...
background_sampling <- rep(c("family", "order"), each=(0.5*params)) 

# mean of eco params
Y <- c(# family level inference
       ...
       fit_summary2$summary[8,1] # psi1 - tree cover
)

# confidence intervals
lower_95 <- c(# family level inference
  ...
  fit_summary2$summary[8,4] # psi1 - tree cover
)

upper_95 <- c(# family level inference
  ...
  fit_summary2$summary[8,8] # psi1 - tree cover
)

# confidence intervals
lower_50 <- c(# family level inference
  ...
  fit_summary2$summary[8,5] # psi1 - tree cover
)

upper_50 <- c(# family level inference
  ...
  fit_summary2$summary[8,7] # psi1 - tree cover
)

df_estimates <- as.data.frame(cbind(X, background_sampling, Y, lower_95, upper_95, lower_50, upper_50))

df_estimates$X <- as.factor(df_estimates$X)
df_estimates$background_sampling <- as.factor(df_estimates$background_sampling)
...
p <- p +
  geom_errorbar(aes(x=X, ymin=lower_95, ymax=upper_95, group=background_sampling, colour=background_sampling),
                position=position_dodge(width=0.5),width=0.1,size=1,alpha=0.5) +
  geom_errorbar(aes(x=X, ymin=lower_50, ymax=upper_50, group=background_sampling, colour=background_sampling),
                position=position_dodge(width=0.5),width=0,size=3,alpha=0.8) +
  geom_point(aes(x=X, y=Y, group=background_sampling, colour=background_sampling), 
...
Y <- c(# family level inference
  ...
  fit_summary2$summary[12,1] # gamma - tree cover
)
...
lower_95 <- c(# family level inference
  ...
  fit_summary$summary[12,4], # gamma - tree cover
  # order level inference
  ...
  fit_summary2$summary[12,4] # gamma - tree cover
)
...
upper_95 <- c(# family level inference
  ...
  fit_summary$summary[12,8], # gamma - tree cover
  # order level inference
  ...
  fit_summary2$summary[12,8] # gamma - tree cover
)
...
lower_50 <- c(# family level inference
  ...
  fit_summary$summary[12,5], # gamma - tree cover
  # order level inference
  ...
  fit_summary2$summary[12,5] # gamma - tree cover
)
...
upper_50 <- c(# family level inference
  ...
  fit_summary$summary[12,7], # gamma - tree cover
  # order level inference
  ...
  fit_summary2$summary[12,7] # gamma - tree cover
)
...
q <- q +
  ...
  geom_errorbar(aes(x=X, ymin=lower_50, ymax=upper_50, group=background_sampling, colour=background_sampling),
                position=position_dodge(width=0.5),width=0,size=3,alpha=0.8) +
  geom_point(aes(x=X, y=Y, colour=background_sampling), 
             position=position_dodge(width=0.5),
             size = 5, alpha = 0.8) 
q
...
Y <- c(# family level inference
  ...
  fit_summary2$summary[18,1] # phi - tree cover
)
...
lower_95 <- c(# family level inference
  ...
  fit_summary$summary[18,4], # phi - tree cover
  # order level inference
  ...
  fit_summary2$summary[18,4] # phi - tree cover
)
...
upper_95 <- c(# family level inference
  ...
  fit_summary$summary[18,8], # phi - tree cover
  # order level inference
  ...
  fit_summary2$summary[18,8] # phi - tree cover
)
...
lower_50 <- c(# family level inference
  ...
  fit_summary$summary[18,5], # phi - tree cover
  # order level inference
  ...
  fit_summary2$summary[18,5] # phi - tree cover
)
...
upper_50 <- c(# family level inference
  ...
  fit_summary$summary[18,7], # phi - tree cover
  # order level inference
  ...
  fit_summary2$summary[18,7] # phi - tree cover
)
