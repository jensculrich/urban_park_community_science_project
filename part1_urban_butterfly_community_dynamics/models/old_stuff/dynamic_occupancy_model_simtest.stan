...
parameters {
  ...
  real psi1_wingspan; // wingspan effect
...
transformed parameters {
  ...
  vector[n_species] mu_psi1_species;
  for(i in 1:n_species){
    mu_psi1_species[i] = psi1_0 + psi1_wingspan*wingspan[i];
  }
...
model {
  ...
  // occupancy
  // initial state
  psi1_species ~ normal(mu_psi1_species, sigma_psi1_species);
  psi1_0 ~ normal(0, 1); 
        ...
        if(sum(V_NA[i,j,k]) > 0){
          ...
          if (sum(V[i,j,k]) > 0){ // lp observed 
            ...
            target += (log_sum_exp(log(psi[i,j,k]) + log1m(p[i,j,k,1])*V_NA[i,j,k,1] + 
                                                     ...
                                                     log1m(p[i,j,k,6])*V_NA[i,j,k,7] + 
                                                     log1m(p[i,j,k,6])*V_NA[i,j,k,8] + 
                                                     log1m(p[i,j,k,6])*V_NA[i,j,k,9] + 
                                                     log1m(p[i,j,k,6])*V_NA[i,j,k,10] + 
                                                     log1m(p[i,j,k,6])*V_NA[i,j,k,11] + 
                                                     log1m(p[i,j,k,6])*V_NA[i,j,k,12],
