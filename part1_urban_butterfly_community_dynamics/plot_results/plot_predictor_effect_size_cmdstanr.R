...
for(i in 1:n_regions){
  ...
  gamma_migratory <- which( rownames(estimates)=="mu_gamma_migratory" )
  ...
  first_gamma_migratory <- which( rownames(estimates)=="gamma_migratory[1]" )
  ...
  for(j in 1:(n_cities-1)){
    ...
    Y[index_lower:index_upper] <- c(
      ...
      estimates[first_gamma_migratory+(j-1),2],  # gamma - migratory
    ...
    lower_95[index_lower:index_upper] <- c(
      ...
      estimates[first_gamma_migratory+(j-1),6],  # gamma - migratory
    ...
    upper_95[index_lower:index_upper] <- c(
      ...
      estimates[first_gamma_migratory+(j-1),7],  # gamma - migratory
    ...
    lower_50[index_lower:index_upper] <- c(
      ...
      estimates[first_gamma_migratory+(j-1),8],  # gamma - migratory
    ...
    upper_50[index_lower:index_upper] <- c(
      ...
      estimates[first_gamma_migratory+(j-1),9],  # gamma - migratory
...
for(i in 1:n_regions){
  ...
  phi_migratory <- which( rownames(estimates)=="mu_phi_migratory" )
  ...
  first_phi_migratory <- which( rownames(estimates)=="phi_migratory[1]" )
  ...
  for(j in 1:(n_cities-1)){
    ...
    Y[index_lower:index_upper] <- c(
      ...
      estimates[first_phi_migratory+(j-1),2],  # phi - migratory
    ...
    lower_95[index_lower:index_upper] <- c(
      ...
      estimates[first_phi_migratory+(j-1),6],  # phi - migratory
    ...
    upper_95[index_lower:index_upper] <- c(
      ...
      estimates[first_phi_migratory+(j-1),7],  # phi - migratory
    ...
    lower_50[index_lower:index_upper] <- c(
      ...
      estimates[first_phi_migratory+(j-1),8],  # phi - migratory
    ...
    upper_50[index_lower:index_upper] <- c(
      ...
      estimates[first_phi_migratory+(j-1),9],  # phi - migratory
...
r <- ggplot(df_estimates) +
   ...
   scale_y_continuous(str_wrap("Posterior model estimate (logit-scaled)", width = 30),
                      limits = c(-4, 10), breaks = c(-8, -6, -4, -2, 0, 2, 4, 6, 8, 10, 12)) +
   guides(color = guide_legend(title = "city")) +
   scale_color_manual(values=my_palette) + 
                                        geom_hline(yintercept = 0, lty = "dashed") +
   ggtitle("Persistence") +
